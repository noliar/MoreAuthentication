...
namespace Microsoft.AspNet.Authentication.Douban
{
    ...
    ///  豆瓣开放平台授权核心处理类
    /// </summary>
    public class DoubanAuthenticationHandler : OAuthAuthenticationHandler<DoubanAuthenticationOptions>
    {
        public DoubanAuthenticationHandler(HttpClient backchannel) : base(backchannel) { }
        ...
        protected override async Task<AuthenticationTicket> CreateTicketAsync(ClaimsIdentity identity, AuthenticationProperties properties, OAuthTokenResponse tokens)
        {
            ...
            var identifier = DoubanAuthenticationHelper.GetId(payload);
            ...
            var name = DoubanAuthenticationHelper.GetName(payload);
            ...
            var avatar = DoubanAuthenticationHelper.GetAvatar(payload);
            if (!string.IsNullOrEmpty(avatar))
            {
                identity.AddClaim(new Claim("urn:douban:avatar", avatar, ClaimValueTypes.String, Options.ClaimsIssuer));
            }

            var uid = DoubanAuthenticationHelper.GetUid(payload);
